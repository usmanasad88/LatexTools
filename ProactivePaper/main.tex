...
For 6-DOF pose estimation and tracking, the Perception Module employs a model-free pipeline built on Any6D~\cite{any6d} and FoundationPose~\cite{foundationpose}. Given only a monocular RGB video and 3D mesh models of the objects of interest, the pipeline operates as follows. First, video frames are extracted and metric depth is estimated for each frame using Depth Anything 3~\cite{depthanything3}, which provides per-pixel depth in metres from a single RGB image. Object segmentation masks are obtained using SAM 3~\cite{sam3, sam3dobjects}, a text-prompted segmentation model that generates per-object masks across all frames without requiring manual bounding box annotations. The object meshes are then automatically scaled to their real-world dimensions by aligning the mesh extent with the depth-derived oriented bounding box of the segmented region. An initial 6-DOF pose is estimated on the first frame via Any6D's registration procedure, which jointly optimises the object's position and orientation using the scaled mesh, the RGB observation, the metric depth map, and the segmentation mask. Since per-frame segmentation masks are available from SAM 3, the same registration procedure is applied independently to each subsequent frame rather than relying on frame-to-frame tracking, which is prone to drift. On each new frame, the pipeline generates random pose hypotheses, refines them against the depth and RGB observations within the masked region, and scores each candidate to select the best pose. This per-frame registration strategy yields accurate 6-DOF poses throughout the video without accumulating temporal errors. Finally, the estimated poses are used to render textured mesh overlays on the original video frames via differentiable rasterization, providing a visual verification of pose estimation quality. Figure~\ref{fig:pose_tracking} shows a representative output frame with mesh overlays for the two bottles and weighing scale.
