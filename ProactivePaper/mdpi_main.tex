...
%=================================================================
% Additional packages (beyond those loaded by the MDPI class)
\usepackage{listings}
...
\Title{A Responsive and Explainable Robotics Solution for Human-Centric Industry 5.0 Applications}

% MDPI internal command: Title for citation in the left column
% Removed: \TitleCitation not defined in mdpi.cls v6.0 (Dec 2025)
%\TitleCitation{A Responsive and Explainable Robotics Solution for Human-Centric Industry 5.0 Applications}
...
\Author{Usman Asad $^{1,}$*\orcidA{}, Azfar Khalid $^{2}$\orcidB{}, Waqas Akbar Lughmani $^{3}$\orcidC{}, Shummaila Rasheed $^{1}$\orcidD{} and Muhammad Mahabat Khan $^{1}$\orcidE{}}
...
\address{%
$^{1}$ \quad Department of Mechanical Engineering, Capital University of Science and Technology, 45750, Islamabad, Pakistan; shummaila@cust.edu.pk (S.R.); drmahabat@cust.edu.pk (M.M.K.)\\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{fig1_gemini.jpg}
    \caption{High-level architecture of the framework showing the flow from perception through decision-making to action execution.\label{fig:architecture}}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Intention.jpg}
    \caption{Example of Intention Monitoring for a task involving weighing of resin and hardener bottles in composite manufacturing\label{fig:intent}}
\end{figure}

\subsubsection{Motion Predictor}

The Motion Predictor forecasts future human movement trajectories. Building on prior work in human motion prediction for HCDTs~\cite{Asad2024_intention}, it generates short-term trajectory predictions (0.5--2 seconds), predicted positions of objects being manipulated, and collision risk assessment for robot path planning. This enables the robot to proactively avoid the human's anticipated path and time its interventions appropriately.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{perception.png}
    \caption{Example of output of perception module for a task involving weighing of resin and hardener bottles in composite manufacturing\label{fig:perception}}
\end{figure}

For 6-DOF pose estimation and tracking, the Perception Module employs a model-free pipeline built on Any6D~\cite{any6d} and FoundationPose~\cite{foundationpose}. Given only a monocular RGB video and 3D mesh models of the objects of interest, the pipeline operates as follows.
Video frames are extracted at a configurable stride and metric depth is estimated for each frame using Depth Anything 3~\cite{depthanything3}, which provides per-pixel depth in metres from a single RGB image. In parallel, SAM~3~\cite{sam3, sam3dobjects} generates per-object segmentation masks for every frame using text prompts, without requiring manual bounding-box annotations. Multiple instances of the same object class are tracked across frames via Hungarian matching on detection-centre proximity.
Rather than initialising from the first video frame---where objects may be partially occluded or far from the camera---the pipeline selects the frame with the largest segmentation mask (in pixels) for each object as the initialisation frame. This maximises the number of valid depth points available for geometry estimation and improves the reliability of the initial pose.
 The 3D mesh model is automatically scaled to real-world dimensions by computing the oriented bounding box (OBB) of the depth-lifted point cloud within the segmentation mask and aligning the mesh extents accordingly. Statistical outlier removal is applied before OBB estimation, with the neighbour count clamped to a minimum of five to handle small masks robustly; if outlier removal eliminates all points, the unfiltered cloud is used as a fallback.
An initial 6-DOF pose is estimated on the best initialisation frame via Any6D's registration procedure, which samples random pose hypotheses, refines each against the depth and RGB observations within the masked region, and selects the best-scoring candidate. Since per-frame segmentation masks are available from SAM~3, the same registration procedure is applied independently to each subsequent frame on which a valid mask exists, rather than relying on frame-to-frame tracking which is prone to drift. When a mask is unavailable for a given frame, the pipeline falls back to Any6D's temporal tracking using the most recent valid pose as a prior, provided that pose is geometrically valid (positive depth, no non-finite values).
The estimated poses are used to render textured mesh overlays on the original video frames via differentiable rasterisation~\cite{nvdiffrast}, providing visual verification of pose estimation quality. Frames for which pose estimation fails or yields a degenerate result are written without overlay rather than causing pipeline failure. Figure~\ref{fig:pose_tracking} shows a representative output frame with mesh overlays for the two bottles.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/pose_tracking_overlay.png}
    \caption{6-DOF pose estimation output from the perception pipeline. Textured mesh overlays rendered on the input RGB frame demonstrate accurate pose estimation of multiple objects (two bottles and a weighing scale) using Any6D~\cite{any6d} with metric depth from Depth Anything 3~\cite{depthanything3} and segmentation from SAM 3~\cite{sam3}.\label{fig:pose_tracking}}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Example of output of the affordance and performance module for a task involving weighing of resin and hardener bottles in composite manufacturing\label{fig:affordance}}
\end{figure}
...
The framework is implemented in Python with key dependencies including LangGraph (state machine for orchestrating the perception-decision-action loop), Google Gemini (VLM for intent prediction and decision reasoning), OpenCV/PIL (image processing), NumPy (numerical computation for trajectories and poses), ROS 2 Humble (robot middleware for real robot deployment), and Isaac Sim (physics simulation for testing and synthetic data). The robot control stack runs as a set of coordinated services managed through a single session launcher, which initializes the UR5 driver, gripper adapter, motion planner, program executor, REST API, and graphical interface as separate windows within one orchestrated session. This architecture allows any subset of capabilities to be activated depending on the operational mode---from fully autonomous program execution to Wizard-of-Oz data collection with human teleoperators.

\subsection{Robot Control and Task Programming Infrastructure}
\label{sec:control_infrastructure}

A central requirement for both proactive assistance and ground-truth data collection is a flexible, multi-modal robot control infrastructure. The system must support rapid authoring of assistance actions, intuitive manual control during Wizard-of-Oz sessions, and an external interface for autonomous agents. We address these requirements through a layered architecture with four complementary input modalities---a domain-specific programming language, immersive VR teleoperation, desktop SpaceMouse control, and a REST API---all converging on a unified ROS~2 control layer. Figure~\ref{fig:control_architecture} summarizes the integration.
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        layer/.style={draw, dashed, rounded corners, inner sep=6pt, fill=#1!8},
        arr/.style={-{Stealth[length=2mm]}, thick}
    ]
        % Input modality boxes
        \node[box, fill=blue!15] (prog) {.prog DSL\\Programs};
        \node[box, fill=green!15, right=of prog] (vr) {Quest 3S\\VR Teleop};
        \node[box, fill=green!15, right=of vr] (sm) {SpaceMouse\\Teleop};
        \node[box, fill=orange!15, right=of sm] (api) {REST API\\(VLA / Agent)};
        
        % Planning layer
        \node[box, fill=yellow!15, below=0.8cm of $(prog)!0.5!(vr)$] (curobo) {cuRobo\\Motion Planner};
        \node[box, fill=yellow!15, below=0.8cm of $(vr)!0.5!(sm)$] (jac) {Analytical\\Jacobian IK};
        \node[box, fill=yellow!15, below=0.8cm of $(sm)!0.5!(api)$] (bridge) {Flask/ROS~2\\Bridge};
        
        % ROS 2 layer
        \node[box, fill=gray!15, below=0.8cm of jac, minimum width=9cm] (ros) {ROS~2 Control Layer (trajectory / velocity controllers)};
        
        % Robot + outputs
        \node[box, fill=red!10, below=0.7cm of ros] (robot) {UR5 + Robotiq 2F-85};
        \node[box, fill=purple!10, left=0.4cm of robot] (ui) {Streamlit UI};
        \node[box, fill=purple!10, right=0.4cm of robot] (rec) {Episode Recorder};
        
        % Arrows
        \draw[arr] (prog) -- (curobo);
        \draw[arr] (vr) -- (jac);
        \draw[arr] (sm) -- (jac);
        \draw[arr] (api) -- (bridge);
        \draw[arr] (curobo) -- (ros);
        ...
        \draw[arr] (ros) -- (rec);
    \end{tikzpicture}
    \caption{Multi-modal robot control architecture. Four input modalities converge on a unified ROS~2 control layer, with two velocity paths: trajectory-based (programs, API) via cuRobo, and streaming joint velocities (VR, SpaceMouse) via analytical Jacobian IK.\label{fig:control_architecture}}
\end{figure}

\subsubsection{Domain-Specific Language for Robot Programs}

To enable rapid authoring of repeatable assistance actions---by operators, researchers, or generative AI---we designed a lightweight domain-specific language (DSL) for robot task programs. Programs are stored as plain-text \texttt{.prog} files, where each line represents one instruction. The DSL supports motion commands (\texttt{movetonamed}, \texttt{movetopose}, \texttt{movetojoint}, \texttt{moverelative}), gripper control (\texttt{opengripper}, \texttt{closegripper}, proportional \texttt{gripper(0.0--1.0)}), timing (\texttt{wait}), velocity scaling (\texttt{set\_speed}), conditional branching (\texttt{if near(Name)}, \texttt{if gripper\_open}, \texttt{else}, \texttt{endif}), and nested sub-program invocation (\texttt{runprogram}). A parser validates balanced control flow and resolves named positions from a configuration file.

Listing~\ref{lst:prog_example} illustrates a representative program for moving a resin bottle from a storage position to the workspace. The program demonstrates conditional checks on gripper and proximity state, named position references, and sequenced pick-and-place logic. Such programs serve as the pre-programmed assistance actions that the AURA decision engine can trigger through the Action Layer.

\begin{lstlisting}[language={}, caption={Example robot program for material handling (abridged).}, label=lst:prog_example]
set_speed(0.5)
if near(Home)
  wait(0.1)
else
...
opengripper
movetonamed(Above_Bottle_Resin_Storage)
movetonamed(Bottle_Resin_Storage)
closegripper
movetonamed(Above_Bottle_Resin_Storage)
movetonamed(Home)
movetonamed(Above_Bottle_Resin_Workspace)
movetonamed(Bottle_Resin_Workspace)
opengripper
movetonamed(Home)
\end{lstlisting}

A dedicated Program Executor node parses, loads, and step-executes these programs, embedding a cuRobo~\cite{Liu2023} motion planner instance for GPU-accelerated, collision-aware trajectory generation. The executor exposes ROS~2 services for loading, executing, pausing, and stopping programs, enabling both interactive use and autonomous triggering by the AURA Brain or external agents. Named positions are maintained in a persistent configuration file and can be appended at runtime---enabling teach-by-demonstration workflows where an operator physically guides the robot (or uses a teleop device) to a location and saves it with a descriptive name for subsequent programmatic use.

The DSL's simplicity makes it amenable to generation by large language models: given a natural-language task description and the list of available named positions, an LLM can produce valid \texttt{.prog} files that the executor runs without modification. This capability is exploited both for rapid prototyping of new assistance behaviors and for the AURA framework's ability to synthesize novel action sequences at runtime.

\subsubsection{Immersive and Desktop Teleoperation}

For Wizard-of-Oz data collection, the robot operator requires a responsive, intuitive control interface that enables timely interventions. We provide two complementary teleoperation modalities.

\textbf{Meta Quest~3S VR Teleoperation.} A ROS~2 node streams the VR controller's 6-DOF pose via ADB at 50~Hz. A DROID-style proportional controller computes Cartesian velocity errors between the controller and end-effector poses, which are converted to joint velocities through an analytical geometric Jacobian derived from the UR5's DH parameters. A damped least-squares pseudo-inverse ($\lambda = 0.05$) ensures numerical stability near singularities. A grip-button deadman switch ensures the robot moves only when the operator intends, while trigger input provides proportional gripper control. Dedicated buttons allow the operator to save the current robot configuration as a named position during operation, enabling on-the-fly capture of task-relevant locations for later use in programs.

\textbf{SpaceMouse Teleoperation.} A 3Dconnexion SpaceMouse provides the same Jacobian-IK velocity pipeline with a desktop form factor. Six-axis inputs are mapped through configurable deadzones and gain scaling, converging on the same \texttt{forward\_velocity\_controller} topic. Buttons provide open/close gripper and save-position functionality.

Both teleoperation modes bypass trajectory planning entirely---publishing joint velocities directly at 50~Hz---yielding smooth, low-latency motion suitable for real-time human-in-the-loop control. A controller-switching utility toggles between the streaming velocity controller (for teleoperation) and the trajectory controller (for program execution and API commands), allowing seamless transitions between operational modes during a session.

\subsubsection{External Control API}

To enable integration with autonomous agents---including Vision-Language-Action (VLA) models and the AURA decision engine---a REST API server exposes the full robot control vocabulary over HTTP. The API provides endpoints for querying available commands (programs, named positions, gripper actions), executing programs, issuing Cartesian and joint-space motions, controlling the gripper, adjusting speed, and saving positions. Each endpoint wraps the corresponding ROS~2 service through a bridge layer, enabling language-agnostic access from any HTTP client.

This API serves a dual role. During development and evaluation, it allows the AURA Brain to dispatch physical actions by issuing HTTP requests to the robot, decoupling the perception-reasoning pipeline from the robot middleware. For broader applicability, it enables VLA models or other learned policies to command the robot through a standardized interface without requiring direct ROS~2 integration, lowering the barrier for researchers working with different software stacks.

\subsubsection{Data Collection and Dataset Pipeline}

An Episode Recorder captures synchronized multimodal data during both teleop and autonomous operation, recording joint states at up to 50~Hz into HDF5 files, workspace video and robot wrist camera (GoPro) streams as MP4, and program execution events as timestamped JSON logs. Metadata including participant identifiers, task descriptions, and session parameters are stored alongside each episode.

Recorded episodes are converted into the LeRobot~\cite{cadene2024lerobot} dataset format for direct compatibility with VLA model training pipelines. The conversion produces 7-dimensional state and action vectors (six joint angles plus gripper position), associates video frames with corresponding robot states at a configurable sampling rate, and derives task descriptions from program filenames. This enables fine-tuning of foundation models such as $\pi_0$~\cite{pi0} or OpenVLA~\cite{openvla} on domain-specific manipulation data collected through the framework.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\authorcontributions{Conceptualization, U.A. and A.K.; methodology, U.A.; software, U.A.; validation, U.A., A.K. and W.A.L.; formal analysis, U.A.; investigation, U.A.; resources, M.M.K.; data curation, U.A.; writing---original draft preparation, U.A.; writing---review and editing, A.K., W.A.L., S.R. and M.M.K.; visualization, U.A.; supervision, A.K., W.A.L. and M.M.K.; project administration, M.M.K.; funding acquisition, M.M.K. All authors have read and agreed to the published version of the manuscript.}
...
\dataavailability{Data will be made available upon publication.}
